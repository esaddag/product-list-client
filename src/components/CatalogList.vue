...
async function fetchData() {
  ...
  if (
    catalogRes.error_message &&
    catalogRes.error_message.includes("The Token has expired")
  ) {
    router.push("/login");
  }
...
async function fetchFilteredData() {
  let params = "";
  let filterKeys = Object.keys(props.msg);
  let filterValues = Object.values(props.msg);
  let i = 0;

  for (i = 0; i < filterKeys.length; i++) {
    if (filterValues[i] != "" && filterValues[i] != null) {
      params = params.concat("&" + filterKeys[i] + "=" + filterValues[i]);
    }
  }
  ...
  if (
    filterRes.error_message &&
    filterRes.error_message.includes("The Token has expired")
  ) {
    router.push("/login");
  }
...
watch(props, () => {
  count.value = props.filterClick.count;
});
watch(count, () => {
  console.log(props.msg);
